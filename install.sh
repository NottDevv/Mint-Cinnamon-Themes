...
header() {
    clear_screen
    echo
    echo -e "  ${CYAN}${BOLD}MINT CINNAMON THEMES${RESET} ${GRAY}v${APP_VERSION}${RESET}"
    echo -e "  ${GRAY}Modern Theme Manager for Linux Mint (Cinnamon)${RESET}"
    echo -e "  ${GRAY}──────────────────────────────────────────────────${RESET}"
    echo
}
...
theme_status() {
    ...
    current_gtk=$(gsettings get org.cinnamon.desktop.interface gtk-theme 2>/dev/null | tr -d "'" || echo "Unknown")
    current_icons=$(gsettings get org.cinnamon.desktop.interface icon-theme 2>/dev/null | tr -d "'" || echo "Unknown")
    current_cursor=$(gsettings get org.cinnamon.desktop.interface cursor-theme 2>/dev/null | tr -d "'" || echo "Unknown")
...
install_menu() {
    while true; do
        ...
        echo -e "  ${GRAY}Choose a theme family to install:${RESET}\n"
...
uninstall_menu() {
    while true; do
        ...
        echo -e "  ${GRAY}Remove managed themes:${RESET}\n"
